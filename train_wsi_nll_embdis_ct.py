...
class Processor():
    ...
    def load_data(self):
        ...
        WSI_info_list, self.survival_time_max, self.survival_time_min = utils.get_WSI_sample_list_ct(
            self.arg.WSI_info_list_file, self.arg.center, self.arg.WSI_patch_ft_dir,
            self.arg.WSI_patch_coor_dir, self.arg.CT_2d_feature_file)# , multi_label=True
    ...
    def train(self, epoch, i_fold, save_model=False):
        ...
        if type(self.arg.device) is list:
            if len(self.arg.device) > 1:
                weights_wsi = OrderedDict(
                    [['module.' + k, v.cuda(self.output_device)] for k, v in weights_wsi.items()])
        ...
        for batch_idx, (features, survival_time, status, sagittal, coronal, coors, id) in enumerate(process):
            if status.sum()==0:
                continue
            with torch.no_grad():
                features = features.float().cuda(self.output_device)
                features_w_noise = 0.99 * features + torch.randn_like(features) * 0.01
            ...
            # forward
            if self.arg.H_coors:
                output, output_fts = self.model(sagittal, coronal, train=True)
            else:
                output, output_fts = self.model(sagittal, coronal, train=True)  # ,wloss ,label

            with torch.no_grad():
                if self.arg.H_coors:
                    output_wsi, output_fts_wsi = self.model_wsi(features, coors, train=False)
                    _, output_fts_wsi_noise = self.model_wsi(features_w_noise, coors, train=False)
                else:
                    output_wsi, output_fts_wsi = self.model_wsi(features, train=False)
                    _, output_fts_wsi_noise = self.model_wsi(features_w_noise, coors, train=False)

                ## L2 dis
                wsi_w = torch.sqrt(torch.sum((output_fts_wsi - output_fts_wsi_noise) ** 2, dim=1))
                wsi_w = wsi_w / wsi_w.sum()
            ...
            ## cosine similarity
            loss_k = (torch.sum((1 - F.cosine_similarity(output_fts, output_fts_wsi, dim=1)) * wsi_w * 0.05)
                      + torch.sum((torch.sum(output_fts - output_fts_wsi, dim=1) ** 2) * wsi_w) * 1e-7)

            loss = self.arg.lamb * loss + (1 - self.arg.lamb) * loss_k
        ...
        for batch_idx, (features, survival_time, status, sagittal, coronal, coors, id) in enumerate(process):
            with torch.no_grad():
                ...
                if isinstance(self.model, torch.nn.DataParallel):
                    if self.arg.H_coors:
                        output, output_fts = self.model.forward(sagittal, coronal, train=True)  # forward(data)# test_
                    else:
                        output, output_fts = self.model.forward(sagittal, coronal, train=True)
                else:
                    if self.arg.H_coors:
                        output, output_fts = self.model.forward(sagittal, coronal, train=True)
                    else:
